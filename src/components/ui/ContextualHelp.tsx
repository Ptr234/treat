...
interface HelpContentItem {
  type: 'tip' | 'info' | 'action' | 'suggestion' | 'step' | 'intro';
  text: string;
}
...
export default function ContextualHelp() {
  ...
  const helpDatabase = useMemo(() => ({
    '/investments': {
      page: {
        ...
        content: [
          { type: 'tip' as const, text: 'Use the sector filter to narrow down investments by industry' },
          { type: 'tip' as const, text: 'Sort by ROI to see the highest return opportunities first' },
          { type: 'info' as const, text: 'Investment Support services offer premium benefits and faster processing' },
          { type: 'action' as const, text: 'Click "Apply Now" to start your investment application process' }
        ]
      },
      noResults: {
        title: 'No Investment Matches Found',
        content: [
          { type: 'suggestion' as const, text: 'Try broadening your search criteria or clearing filters' },
          { type: 'suggestion' as const, text: 'Consider related sectors - many investments span multiple industries' },
          { type: 'action' as const, text: 'Contact our investment advisors for personalized recommendations' }
        ]
      },
      firstVisit: {
        title: 'Welcome to Investment Opportunities',
        content: [
          { type: 'intro' as const, text: 'Uganda offers investment opportunities with 15-35% ROI potential' },
          { type: 'tip' as const, text: 'Start by exploring sectors that match your investment goals and experience' },
          { type: 'tip' as const, text: 'Use our ROI calculator to project potential returns on your investment' }
        ]
      }
    },
    '/services': {
      page: {
        ...
        content: [
          { type: 'tip' as const, text: 'Search by service name, agency, or keywords for faster results' },
          { type: 'info' as const, text: 'Investment Support services provide premium assistance for serious investors' },
          { type: 'tip' as const, text: 'Use the category filter to browse services by business type' }
        ]
      },
      businessRegistration: {
        title: 'Business Registration Guide',
        content: [
          { type: 'step' as const, text: '1. Choose your business structure (Company, Partnership, etc.)' },
          { type: 'step' as const, text: '2. Reserve your company name with URSB' },
          { type: 'step' as const, text: '3. Prepare required documents and complete application' },
          { type: 'step' as const, text: '4. Pay registration fees and await certificate' }
        ]
      }
    },
    '/agencies': {
      page: {
        ...
        content: [
          { type: 'tip' as const, text: 'Each agency specializes in different business aspects - choose the right one for your needs' },
          { type: 'info' as const, text: 'Contact information includes direct phone numbers and email addresses' },
          { type: 'tip' as const, text: 'Many agencies offer online services to save you time and travel' }
        ]
      }
    }
  }), []);
  ...
  useEffect(() => {
    ...
    const pageHelp = helpDatabase[currentPath as keyof typeof helpDatabase]?.page;
  ...
  useEffect(() => {
    ...
    const currentHelp = helpDatabase[currentPath as keyof typeof helpDatabase];
    if (multipleSearches && noResults && currentHelp && 'noResults' in currentHelp) {
      setHelpContent(currentHelp.noResults);
      setIsVisible(true);
    }
    
    // User is looking at business registration services
    const viewingBusinessServices = recentActions.some(a => 
      a.action === 'view_service' && 
      a.data && 
      typeof a.data === 'object' && 
      'category' in a.data && 
      typeof a.data.category === 'string' && 
      a.data.category.includes('Business Registration')
    );
    
    if (viewingBusinessServices && currentHelp && 'businessRegistration' in currentHelp) {
      setHelpContent(currentHelp.businessRegistration);
      setIsVisible(true);
    }
  }, [userActions, pathname, helpDatabase]);

  // Global help trigger for complex forms or errors
  useEffect(() => {
    const handleGlobalHelp = (event: CustomEvent) => {
    ...
    window.addEventListener('contextual-help', handleGlobalHelp as EventListener);
    return () => window.removeEventListener('contextual-help', handleGlobalHelp as EventListener);
  }, []);
  ...
  useEffect(() => {
    (window as unknown as { trackHelp?: typeof trackUserAction }).trackHelp = trackUserAction;
    return () => {
      delete (window as unknown as { trackHelp?: typeof trackUserAction }).trackHelp;
    };
  }, [trackUserAction]);
