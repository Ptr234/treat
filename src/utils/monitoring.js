...
export class MonitoringManager {
  ...
  setupErrorRateMonitoring() {
    ...
    window.addEventListener('error', () => {
      errorCount++
      this.recordMetric('errors', 'javascript_error_rate', (errorCount / Math.max(totalRequests, 1)) * 100)
    })

    // Track unhandled promise rejections
    window.addEventListener('unhandledrejection', () => {
      errorCount++
      this.recordMetric('errors', 'promise_rejection_rate', (errorCount / Math.max(totalRequests, 1)) * 100)
    })

    // Track successful requests (approximation)
    setInterval(() => {
      totalRequests++
      this.recordMetric('errors', 'error_rate', (errorCount / totalRequests) * 100)
  ...
  checkMetricAlerts(metricKey, value, _metadata) {
    const alertRules = {
      ...
      'errors.error_rate': { threshold: 5, condition: 'greater', severity: 'critical' }
    }
