...
export class EnhancedCacheManager {
  ...
  async startBackgroundCacheWarming() {
    ...
    setTimeout(async () => {
      try {
        // Only use safe, local URLs for cache warming
        const warmupUrls = [
          '/investments',
          '/services', 
          '/tools',
          '/support'
        ].filter(url => this.isValidCacheUrl(url))
        ...
        for (const url of warmupUrls) {
          try {
            const response = await fetch(url, { 
              ...
              cache: 'no-cache', // Prevent cache confusion
              headers: {
  ...
  showCacheNotification(message, type = 'info') {
    ...
    notification.className += ` ${colors[validType]}`
    ...
    icon.textContent = validType === 'success' ? '✅' : validType === 'error' ? '❌' : validType === 'warning' ? '⚠️' : 'ℹ️'
