...
export default function NavigationGuide() {
  ...
  const [currentGuide, setCurrentGuide] = useState<NavigationGuide | null>(null);
  const [hasSeenGuide, setHasSeenGuide] = useState<Record<string, boolean>>({});
  const pathname = usePathname();
  const router = useRouter();

  // Navigation guidance content for each page
  const navigationGuides = useMemo((): Record<string, NavigationGuide> => ({
    '/': {
  ...
  useEffect(() => {
    ...
    const guide = currentPath ? navigationGuides[currentPath] : null;
    
    if (guide && currentPath && !hasSeenGuide[currentPath]) {
  ...
  }, [pathname, hasSeenGuide, navigationGuides]);
  ...
  const handleDismiss = () => {
    setIsVisible(false);
    if (pathname) {
      const newSeen = { ...hasSeenGuide, [pathname]: true };
  ...
  const handleNextAction = () => {
    if (currentGuide?.nextBestAction) {
      window.location.href = currentGuide.nextBestAction.path;
      handleDismiss();
    }
  };
